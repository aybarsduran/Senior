...
namespace Toolbox.Editor.Internal
{
    ...
    public abstract class ReorderableListBase
    {
        ...
        public string GetElementDisplayName(SerializedProperty element, int index)
        {
            ...
            //try to override name using customized child label
            if (ElementLabel == null)
                return elementName;
            
            var childProp = element.FindPropertyRelative(ElementLabel);
            return childProp != null ? GetLabelByValue(childProp) : GetElementDefinedName(index);
        }
        
        public string GetLabelByValue(SerializedProperty property)
        {
                ...
                case SerializedPropertyType.LayerMask:
                    switch (property.intValue)
                    {
                        ...
                        case ~0:
                            return "Everything";
                        default:
                            return LayerMask.LayerToName((int)Mathf.Log(property.intValue, 2));
                    }
